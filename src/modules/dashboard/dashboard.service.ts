...
import { FinancialRecordsService, FieldFinancialSummary } from '../financial/financial-records.service';
...
export class DashboardService {
  ...
  async getSummary(ownerId: string): Promise<DashboardSummary> {
    ...
    const [weatherOverview, recentActivities, activeAlerts, financialSummaries] =
      await Promise.all([
  ...
  private async getWeatherOverview(fields: Field[]): Promise<WeatherOverviewItem[]> {
    if (!fields.length) {
      return [];
    }
  ...
  private async getRecentActivities(fieldIds: string[]): Promise<FieldActivity[]> {
    if (!fieldIds.length) {
      return [];
    }
