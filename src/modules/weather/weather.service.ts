...
export class WeatherService {
  ...
  constructor(
    ...
    const weatherConfig = this.configService.get<AppConfiguration['weather']>(
      'weather',
    );
  ...
  async getForecast(
    ...
    const field = await this.fieldAccessService.getOwnedField(fieldId, ownerId);
    const { lat, lng } = this.extractCoordinates(field);
    const forecastDays = query.days ?? this.weatherConfig.defaultForecastDays;
    const apiData = await this.fetchForecastFromProvider(lat, lng);
    const normalizedForecast = this.normalizeForecast(apiData, forecastDays);

    const saved = await Promise.all(
      normalizedForecast.map((reading) => this.persistReading(field, reading)),
    );
    await Promise.all(
  ...
  private normalizeForecast(
    ...
    for (const entry of list) {
      const timestamp = typeof entry.dt === 'number' ? entry.dt * 1000 : Date.now();
  ...
  private async persistReading(
    ...
    return {
      ...
      temperatureC: saved.temperatureC ? Number(saved.temperatureC) : undefined,
  ...
  private extractCoordinates(field: Field): { lat: number; lng: number } {
    ...
    if (
      ...
      throw new BadRequestException('Field boundary does not contain valid coordinates');
    }
    return centroid;
  }
  ...
  private handleProviderError(error: unknown, context: string) {
    if ((error as AxiosError).isAxiosError) {
      ...
      this.logger.error(`Failed to fetch ${context} weather`, (error as Error).stack);
    }
  }
}
